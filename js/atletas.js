...
export function renderAtletas(nome){
  ...
  const rows=p.atletas.map((a,i)=>`<tr>
    <td><input type="text" value="${LNE.esc(a.nome)}" onchange="updAtl(${LNE.jstr(nome)},${i},'nome',this.value)" placeholder="Nome"/></td>
    <td><input type="text" value="${LNE.esc(a.categoria||'')}" onchange="updAtl(${LNE.jstr(nome)},${i},'categoria',this.value)" placeholder="A12" style="width:65px;"/></td>
    <td><input type="text" value="${LNE.esc(a.escola||'')}" onchange="updAtl(${LNE.jstr(nome)},${i},'escola',this.value)" placeholder="Escola"/></td>
    <td style="text-align:center;">
      <label style="display:inline-flex;align-items:center;gap:4px;cursor:pointer;font-size:11px;">
        <input type="checkbox" ${a.federado?'checked':''} onchange="updAtl(${LNE.jstr(nome)},${i},'federado',this.checked)" style="width:14px;height:14px;accent-color:var(--azm);"/>
    ...
    <td><button class="btn b-red" style="font-size:10px;padding:2px 7px;" onclick="rmAtl(${LNE.jstr(nome)},${i})">✕</button></td>
  </tr>`).join('');
  el.innerHTML=`<div class="card" style="margin-bottom:12px;">
    ...
    <div id="atlbody-${LNE.tid(nome)}" class="coll-body" style="max-height:${collapsed?'0px':'9999px'};">
      ...
      <div class="card-ft"><button class="btn b-out" onclick="addAtleta(${LNE.jstr(nome)})">+ Atleta</button></div>
    </div></div>`;
}
...
export function addAtleta(nome){
  ...
  document.getElementById('addAtletaConteudo').innerHTML=`
    ...
    <div style="display:flex;gap:8px;justify-content:flex-end;margin-top:10px;">
      ...
      <button class="btn b-pri" onclick="confirmarAddAtleta(${LNE.jstr(nome)})">+ Adicionar</button>
    </div>`;
